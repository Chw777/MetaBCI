...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        return output
class tmransnet(nn.Module):
    def __init__(self, num_classes=2, num_samples=1000, num_channels=58, embed_dim=40, pool_size=50,
                 pool_stride=15, num_heads=8, fc_ratio=4, depth=4, attn_drop=0.5, fc_drop=0.5):
        super().__init__()
        # 修正卷积层定义，使拼接后的通道数与 embed_dim 一致
        self.temp_conv0 = nn.Conv2d(1, embed_dim // 5, (1, 1), padding=(0, 0))
        self.temp_conv1 = nn.Conv2d(1, embed_dim // 5, (1, 15), padding=(0, 7))
        self.temp_conv2 = nn.Conv2d(1, embed_dim // 5, (1, 25), padding=(0, 12))
        self.temp_conv3 = nn.Conv2d(1, embed_dim // 5, (1, 51), padding=(0, 25))
        self.temp_conv4 = nn.Conv2d(1, embed_dim // 5, (1, 65), padding=(0, 32))
        self.bn1 = nn.BatchNorm2d(embed_dim)  # 现在拼接后的通道数为 embed_dim (40)
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 3:
            x = x.unsqueeze(1)
        x0 = self.temp_conv0(x)  # 修正：使用 temp_conv0 而非重复使用 temp_conv1
